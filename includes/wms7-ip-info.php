...
function wms7_who_is( $ip_or_dns, $who_is ) {
	...
	switch ( $who_is ) {
		case "IP-API":
			$result = wms7_ip_api( $user_ip );
			break;
		case "IP-Info":
			$result = wms7_ip_info( $user_ip );
			break;
		case "SxGeo":
			$result = wms7_sx_geo( $user_ip );
			break;
		case "Geobytes":
			$result = wms7_geobytes( $user_ip );
			break;
		case "none":
...
/**
 * Description: Retrieve ip information from the provider IP_API.
...
/**
 * Description: Retrieve ip information from the provider IP_info.
 * @param  string $user_ip User ip.
 * @return array In format json.
 */
...
/**
 * Description: Retrieve ip information from the provider SxGeo.
 * @param  string $user_ip User ip.
 * @return array In format json.
 */
...
/**
 * Description: Retrieve ip information from the provider Geobytes.
 * @param  string $user_ip User ip.
 * @return array In format json.
 */
function wms7_geobytes( $user_ip ) {
	...
	// timezone.
	if ( "" !== $lat && "" !== $lon ) {
		$str_info["timezone"] = wms7_getClosestTimezone( $lat, $lon );
	} else {
		$str_info["timezone"] = "";
	}
...
/**
 * Description: Retrieve information of country of visitor.
...
function wms7_country_name( $country_code ) {
	...
	$results = $wpdb->get_var(
		$wpdb->prepare(
			"
      SELECT `name`
      FROM {$wpdb->prefix}wms7_countries
      WHERE `code` = %s
      ",
			$country_code
		)
...
/**
 * Description: Attempts to find the closest timezone by coordinates.
 * @param  string $lat Latitude.
 * @param  string $lon Longitude.
 * @return string Timezone.
 */
function wms7_getClosestTimezone( $lat, $lon ) {
	$diffs = array();
